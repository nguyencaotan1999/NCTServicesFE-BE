...
{
    public class AddProductByAdmin : IRequest<Result<bool>>
    {
        public ProductRequest ProductRequest { get; set; }
        public AddProductByAdmin(ProductRequest product)
        {
            ProductRequest = product; 
        }  

    }
    public class AddProductByAdminHandler : IRequestHandler<AddProductByAdmin, Result<bool>>
    {
        private readonly IApplicationWriteDbConnection _sqlDbConnection;
        public AddProductByAdminHandler(IApplicationWriteDbConnection writeDbConnection)
        {
            _sqlDbConnection = writeDbConnection;
        }
        public async Task<Result<bool>> Handle(AddProductByAdmin request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                var response = await _sqlDbConnection.QueryAsync<bool>(SQLConstant.AddProductByAdmin, CommandType.Text, parameters);
                return await Result<bool>.SuccessAsync(response);
            }
            catch (Exception)
            {
                return await Result<bool>.FailAsync("Added Failed");
            }
        }
    }
}
